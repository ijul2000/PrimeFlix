...
// Bila membaca sesi, localStorage diutamakan tapi jatuh balik ke
// cookie kalau localStorage kosong/gagal.
// NOTA: ini bukan penyelesaian mutlak — kalau pelayar sekat KEDUA-DUA
// localStorage & cookie (contohnya sesetengah pelayar dalam-app dalam
// mod storan tidak-berterusan), tiada cara bypass dari kod client-side;
// itu sekatan platform (bukan sekatan laman web ini).
// =========================================================
...
function writeSession(user) {
  ...
  // Sentiasa turut simpan sebagai cookie juga — walaupun localStorage
  // nampak berjaya pada halaman semasa, cookie membantu kalau ia tak
  // kekal lepas navigasi (isu biasa pada sesetengah pelayar dalam-app).
...
// sesi tersimpan (localStorage atau cookie sandaran, disimpan oleh
// login.js), hantar pengguna ke login.html dahulu — bawa URL semasa
// sebagai ?redirect= supaya selepas berjaya log masuk, pengguna
// dibawa balik ke sini.
(function requireAuth() {
  ...
  if (params.get('_ac') === '1') {
    ...
    // pelayar/peranti ini (biasanya pelayar dalam-app WhatsApp/
    // Instagram/Facebook/TikTok dalam mod storan tidak-berterusan).
    // Jangan redirect balik ke login.html (akan loop tanpa henti) —
    // papar mesej jelas supaya pengguna faham punca sebenar.
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* =========================================================
     ...
     (login.html & login.js) — lihat requireAuth() serta
     readSession()/writeSession()/clearSession() (dengan cookie
     fallback) di bahagian atas fail ini.
     ========================================================= */
